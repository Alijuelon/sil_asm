  ...
  <div class="space-y-6 pb-10">
    ...
    <div v-if="isSamuel" class="bg-[#151e32] rounded-2xl p-6 border border-gray-800 shadow-lg mt-6">
      ...
      <div v-else class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        <div v-for="topik in customTopics" :key="topik.nama" class="bg-[#0f172a] border border-gray-700 p-5 rounded-xl">
    ...
    <div v-else class="bg-[#151e32] rounded-2xl p-6 border border-gray-800 shadow-lg mt-6">
      <h2 class="text-lg font-bold text-teal-400 mb-4 flex items-center gap-2"><span>📖</span> Progres Hafalan Wajib (Kumulatif)</h2>
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div class="bg-[#0f172a] border border-gray-700 p-5 rounded-xl">
          ...
          <div class="flex justify-between text-sm mb-2">
            <span class="text-gray-400">Sudah Setor:</span>
...
// === IDENTIFIKASI USER (Aman dari Huruf Besar/Kecil) ===
...
const fetchDashboardData = async () => {
  ...
  // 2. Hitung Kehadiran (Sesuai Kelas Masing-masing)
  ...
  if (isSamuel.value) {
    // LOGIKA SAMUEL: Hitung topik kustom per hari ini
    const { data: hafalan } = await supabase
      .from('hafalan')
      .select('jenis_hafalan, status_nilai')
      .in('student_id', studentIds)
      .eq('hari', selectedDay.value)
      .neq('status_nilai', 'Belum') // Hanya hitung yang sudah dinilai (A/B/C)
    ...
    // LOGIKA ALI: Hitung Titah & Haporseaon (Kumulatif semua hari)
    const { data: hafalan } = await supabase
      .from('hafalan')
